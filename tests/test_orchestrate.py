...
def test_parse_document_converts_docx_then_mineru(tmp_path: Path, monkeypatch) -> None:
    src = tmp_path / "notes.docx"
    src.write_bytes(b"PK")
    converted = tmp_path / "office" / "notes.pdf"
    converted.parent.mkdir(parents=True)
    converted.write_bytes(b"%PDF-1.4\n")

    def fake_convert(input_path, work, allowed=None):
        assert input_path == src
        return converted

    def fake_mineru(pdf_path, out_dir, formula=None, table=None):
        assert pdf_path == converted
        md = tmp_path / "notes.md"
        md.write_text("# Notes\n\nbody\n", encoding="utf-8")
        return md

    monkeypatch.setattr("src.orchestrate.convert_office_to_pdf", fake_convert)
    monkeypatch.setattr("src.orchestrate._mineru_pdf_to_markdown", fake_mineru)

    result = parse_document(src, tmp_path / "parse")
    corpus = json.loads(result.read_text(encoding="utf-8"))
    assert corpus["nodes"][0]["title"] == "Notes"
