...
from .office import WORD_SUFFIXES, convert_office_to_pdf
    ...
    if not name.lower().endswith(".pdf"):
        ...
        if suffix in WORD_SUFFIXES:
            input_path = convert_office_to_pdf(
                input_path, work / "office", allowed=WORD_SUFFIXES | {".pdf"}
            )
            name = input_path.name
        else:
            raise RuntimeError(
                f"Cannot parse {input_path}: provide PDF / Word / markdown / *_corpus.json"
            )
