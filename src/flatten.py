...
def flatten_for_export(corpus: dict[str, Any], document_id: str) -> list[dict[str, Any]]:
    if corpus.get("source_type") == "excel" or (
        isinstance(corpus.get("chunks"), list) and not corpus.get("nodes")
    ):
        return flatten_excel(corpus, document_id)
...
def flatten_excel(corpus: dict[str, Any], document_id: str) -> list[dict[str, Any]]:
    points: list[dict[str, Any]] = []
    ...
    for index, chunk in enumerate(chunks):
        ...
        if not text:
            continue
        raw_id = str(chunk.get("id") or f"row_{index}")
        chunk_id = raw_id if raw_id.startswith(f"{document_id}:") else f"{document_id}:{raw_id}"
        metadata = dict(chunk.get("metadata") or {})
        metadata.setdefault("chunk_type", metadata.get("sheet_kind") or "excel_row")
        points.append({"id": chunk_id, "text": text, "metadata": metadata})
    return points
